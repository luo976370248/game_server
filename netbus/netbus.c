...
void init_server_netbus(int socket_type, int protocal_type) {
...
void session_send_json_cmd_to_service(struct session* s, int to_stype, json_t* cmd) {
...
unsigned int netbus_add_timer(void(*on_timer)(void* udata),void* udata, float after_sec) {
	return add_timer(GATEWAY_TIMER_LIST, on_timer, udata, after_sec);
}

void netbus_cancel_timer(unsigned int timeid) {
	cancel_timer(GATEWAY_TIMER_LIST, timeid);
}

unsigned int netbus_schedule(void(*on_timer)(void* udata),void* udata, float after_sec)
{
