...
extern void socket_send_data(void* ud, unsigned char* buf, int nread);
...
extern void on_connect_lost_entry(struct session* s);
...
static void ws_send_data(struct session* s, const unsigned char* pkg_data, unsigned int pkg_len) {
	int long_pkg = 0;
...
static void tcp_send_data(struct session* s, const unsigned char* body, int len) {
	int long_pkg = 0;
