...
static int select_redis_dbid(int dbid) {
	int ret = -1;
	if (dbid != 0) {
...
void  connect_to_center() {
	if (mysql_center == NULL){
		...
		if (!mysql_real_connect(mysql_center, CENTER_DB_CONF.mysql_ip, CENTER_DB_CONF.mysql_name, CENTER_DB_CONF.mysql_pwd, CENTER_DB_CONF.database_name, CENTER_DB_CONF.mysql_port, NULL, 0)) {
			...
			// 启动一个定时器
			netbus_add_timer(connect_to_center, NULL, 3.0f);
			// end 
		}
		else {
	...
	if (redis_center == NULL) {
		...
		if (redis_center->err != 0) {
			...
			if (CENTER_DB_CONF.redis_pwd != NULL) {
				//有密码的情况
...
int get_uinfo_by_uid(unsigned int uid, struct user_info* uinfo) {
	...
	char* sql_str = "select unick,phone_number,is_guest,uface,usex,from uinfo where id = %d limit 1";
...
int get_uinfo_by_uname_upwd(char* uname, char* upwd, struct user_info* uinfo) {
	...
	char* sql_str = "select id, unick,phone_number,is_guest,uface,usex from uinfo where uname = \"%s\" and upwd =  \"%s\" and is_guest = 0 limit 1";
...
static void update_user_upwd_with_value(unsigned int uid, char* new_upwd_md5) {
...
int get_uinfo_inredis(unsigned uid, struct user_info* uinfo) {
	if (uinfo == NULL) {
		return -1;
	}
	int ret = -1;
